...
def run_tk_solver_gui(puzzle, N, br, bc, cell_size=None, show_numbers=True):
    ...
    def generate_puzzle():
        nonlocal current_grid, N, br, bc, initial_givens, rect_ids, text_ids
    ...
    def solver_worker():
        nonlocal start_time
        try:
            rows_list, total_candidates = build_rows_list(current_grid, N, br, bc, threshold=build_threshold)
        except MemoryError as e:
            root.after(0, lambda: messagebox.showerror('Error', str(e)))
            root.after(0, lambda: start_btn.config(state='normal'))
            return
        # update status: building DLX
        root.after(0, lambda: status_var.set(f'Building DLX (0/{len(rows_list)})'))
        root.after(0, lambda: time_var.set('0 ms (0.00 sec)'))

        num_cols = 4 * N * N
        dlx = DLX(num_cols)
        # allow abort while building DLX rows and update progress occasionally
        total_rows = len(rows_list)
        update_interval = max(1, total_rows // 200)  # ~200 updates
        for idx, (row_id, cols) in enumerate(rows_list, start=1):
            if stop_event.is_set():
                root.after(0, lambda: start_btn.config(state='normal'))
                root.after(0, lambda: status_var.set('Aborted'))
                return
            dlx.add_row(row_id, cols)
            if idx % update_interval == 0 or idx == total_rows:
                root.after(0, lambda i=idx, t=total_rows: status_var.set(f'Building DLX ({i}/{t})'))

        # DLX built, start search
        root.after(0, lambda: status_var.set('Searching...'))

        # ensure recursion limit is high enough for large puzzles
        try:
            needed = N * N + 1000
        ...
        try:
            sol_nodes = algorithm_x_search(dlx, [], stop_event)
        except SearchStopped:
            # aborted: show elapsed time if available
        ...
        if sol_nodes:
            # reconstruct grid (start from initial givens)
            sol_grid = [row[:] for row in initial_givens]
            ...
            root.after(0, lambda: messagebox.showinfo('No solution', 'No solution was found'))
            root.after(0, lambda: start_btn.config(state='normal'))
            root.after(0, lambda: status_var.set('No solution'))
...
def main():
    ...
    puzzle_generated = puzzle_from_solution(sol, remove_rate=args.remove_rate)
    if args.gui:
