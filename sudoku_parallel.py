...
def solve_backtracking(puzzle, N, br, bc, stop_event=None, time_limit=None):
    """Fast backtracking solver using bitmask constraints and MRV heuristic.
    ...
    for r in range(N):
        for c in range(N):
            ...
            if v != 0:
                if not (1 <= v <= N):
                    raise ValueError(f'Cell value out of range at {(r,c)}: {v}')
                bit = 1 << (v - 1)
    ...
    # ensure recursion limit is large enough for big grids
    try:
        needed = max(10000, remaining + 200)
        if sys.getrecursionlimit() < needed:
            sys.setrecursionlimit(needed)
    except Exception:
        pass

    def candidates_mask(rr, cc):
        bb = (rr // br) * numBoxesHoriz + (cc // bc)
        return full_mask & ~(row_mask[rr] | col_mask[cc] | box_mask[bb])

    # propagate naked singles; record assigned cells into `assigned` list
    def propagate(assigned):
        ...
        while progress:
            ...
            for (rr, cc) in empties:
                ...
                if mask & (mask - 1) == 0:
                    vbit = mask
                    grid[rr][cc] = vbit.bit_length()
                    row_mask[rr] |= vbit
                    col_mask[cc] |= vbit
                    box_mask[(rr // br) * numBoxesHoriz + (cc // bc)] |= vbit
                    assigned.append((rr, cc, vbit))
                    remaining -= 1
                    progress = True
        return True
    ...
    # initial propagation
    init_assigned = []
    if not propagate(init_assigned):
        return None

    # recursive search with MRV
    def search():
        ...
        # choose empty cell with fewest candidates (MRV)
        best_mask = 0
        best_count = N + 1
        best_rc = None
        for (rr, cc) in empties:
        ...
        rr, cc = best_rc
        b = (rr // br) * numBoxesHoriz + (cc // bc)
        m = best_mask
        # try candidates (iterate low-bit first)
        while m:
            vbit = m & -m
            m -= vbit
            # assign
            ...
            # backtrack assigned cells
            for ar, ac, av in reversed(assigned):
                grid[ar][ac] = 0
                row_mask[ar] ^= av
                col_mask[ac] ^= av
                box_mask[(ar // br) * numBoxesHoriz + (ac // bc)] ^= av
                remaining += 1
...
def run_tk_solver_gui(puzzle, N, br, bc, cell_size=None, show_numbers=True):
    ...
    def solver_worker():
        ...
        try:
            ...
            sol_grid = solve_backtracking(current_grid, N, br, bc, stop_event)
        except SearchStopped:
